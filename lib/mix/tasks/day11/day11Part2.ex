defmodule Mix.Tasks.AOC.Day11Part2 do
  use Mix.Task
  ...
  def paint_hull(instructions_string) do
    instructions = parse_instructions(instructions_string)
    paint_hull_helper([1], {[], 0, {0, 0}}, {:running, 0, instructions, nil, 0, 0})
  end

  def find_last_coordinate(coordinates, coordinate) do
    coordinates
    |> Enum.reverse()
  ...
  def print_hull(coordinates_with_colors) do
    ...
    min_y = Enum.min(y_coordinates)
    max_y = Enum.max(y_coordinates)
  ...
  def run(_) do
    ...
    hull = paint_hull(instructions_string)
    print_hull(hull)

    # Enum.map(hull, fn c -> elem(c, 0) end)
    # |> IO.inspect()
    # |> MapSet.new()
    # |> MapSet.size()
    # |> IO.inspect()
