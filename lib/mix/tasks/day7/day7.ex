defmodule Mix.Tasks.AOC.Day7 do
  ...
  def runInstruction({99, _, _, _}, _, instructions, _, output) do
    {:halted, 0, instructions, output}
  end

  def runInstruction({operation, mode_1, mode_2, _}, index, instructions, _, output) when operation in [5, 6] do
    ...
    {:running, new_index, instructions, output}
  end

  def runInstruction({operation, mode_1, mode_2, _}, index, instructions, _, output) when operation in [1, 2, 7, 8] do
    ...
    {:running, index + skip_forward, new_instructions, output}
  end

  def runInstruction({3, _, _, _}, index, instructions, input, output) do
    parameter = Enum.at(instructions, index + 1)
    new_instructions = List.replace_at(instructions, parameter, input)
    {:running, index + 2, new_instructions, output}
  end

  def runInstruction({4, mode_1, _, _}, index, instructions, _, _) do
    parameter = Enum.at(instructions, index + 1)
    output = if mode_1 == 0, do: Enum.at(instructions, parameter), else: parameter
    IO.puts("Output: #{output}")
    {:running, index + 2, instructions, output}
  end

  def step_through_program({:running, index, instructions, output}, input) do
    instructions
    |> calculate_operation(index)
    |> runInstruction(index, instructions, input, output)
  end

  def execute_program({:halted, _, _, output}, _) do
    output
  end
  ...
  def run_program(input, program) do
    ...
    execute_program({:running, 0, instructions, nil}, input)
  end
